...
function get_list_product_by_cart_id($cat_id)
{
    global $list_product;

    $result = array(); // Mảng chứa danh sách sản phẩm theo cart_id

    foreach ($list_product as $item) {
        if ($item['cart_id'] == $cat_id) {
            $item['url'] = "?mod=product&act=detail&id={$item['id']}"; // Thêm url vào bên trong $item
            $result[] = $item;
        }
    }
    return $result;
